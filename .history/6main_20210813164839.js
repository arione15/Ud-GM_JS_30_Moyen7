...
eletMdp.addEventListener("blur", function(){
    ...
    if(!eletMdp.value.match(carDecimal)){
        err+="Le mot de passe doit contenit un chiffre !";
    }
    ...
    if(!eletMdp.valu.length < 6){
        err+="Le mot de passe est trop court : il faut au moins 6 caractères !";
    }
    
    if(!eletMdp.valu.length > 8){
        err+="Le mot de passe est trop long : il faut au moins 6 caractères !";
    }
})