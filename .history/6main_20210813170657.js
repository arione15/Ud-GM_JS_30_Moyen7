...
eletMdp.addEventListener("blur", function(){
    ...
    if(eletMdp.value.length < 6){
        err+="<li> est trop court : il faut au minimum 6 caractères ! </li>";
    }
    
    if(eletMdp.value.length > 8){
        err+="<li>est trop long : il faut au maximum 8 caractères !</li>";
    }

    if(err !==""){
        eletAffichage.innertextContent="Le mot de passe :  <u>" + err + "</u>";
        eletAffichage.style.border="2px solid red";
    }else{
        eletAffichage.textContent="Le mot de passe est valide !";
        eletAffichage.style.border="2px solid green";
    }
});